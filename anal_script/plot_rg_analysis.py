...
def plot_rg_analysis(csv_file):
    """
    绘制回转半径 (Rg) 分析图
    
    参数:
        csv_file: 合并后的 Rg 数据 CSV 文件
    """
    ...
    ax.set_title('Radius of Gyration Analysis', fontsize=18)
    ...
    print(f"Rg plot saved.：{output_file}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法：python plot_rg_analysis.py <csv_file>")
        sys.exit(1)
    
    csv_file = sys.argv[1]
    plot_rg_analysis(csv_file)
